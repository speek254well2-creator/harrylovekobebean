...
export default function LongVideoGrid() {
  ...
  const subCategories = ['全部', 'Chairman椅人', 'Podcast今天來學', '阿玩旅遊', '商業形象影片', '旅遊節目'];
  ...
  const filteredProjects = PROJECTS
    .filter(p => {
      const isLongVideo = p.mainCategory === '長影片';
      const matchSub = subFilter === '全部' || p.subCategory === subFilter;
      return isLongVideo && matchSub;
    })
    .sort((a, b) => {
      if (subFilter !== '全部') return 0;
      const orderA = categoryOrder[a.subCategory] || 99;
      const orderB = categoryOrder[b.subCategory] || 99;
      return orderA - orderB;
    });

  if (filteredProjects.length === 0 && subFilter === '全部') return null;
  ...
  return (
    <section id="long-videos" className="py-24 px-6 md:px-12 max-w-7xl mx-auto border-t border-white/5">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
        <div>
          <h2 className="text-4xl md:text-5xl font-bold mb-4">長影片</h2>
          <p className="text-zinc-400 max-w-md">
            深度敘事與品牌形象，用影像說出最動人的故事。
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <AnimatePresence mode="popLayout">
          {filteredProjects.map((project) => (
            ...
            >
              <div className="relative overflow-hidden rounded-xl mb-4 bg-zinc-900 aspect-video">
                {getYouTubeID(project.videoUrl) ? (
                  <div className="w-full h-full pointer-events-none">
                    <iframe
                      src={`https://www.youtube.com/embed/${getYouTubeID(project.videoUrl)}?controls=0&modestbranding=1&rel=0&showinfo=0&mute=1`}
                      width="100%"
                      height="100%"
            ...
            <div className="w-full max-w-6xl flex flex-col gap-8">
              <div className="relative aspect-video rounded-2xl overflow-hidden bg-zinc-900 shadow-2xl border border-white/10">
                {modalVideo.videoUrl.includes('drive.google.com') ? (
                  ...
                  />
                ) : getYouTubeID(modalVideo.videoUrl) ? (
                  <iframe
                    src={`https://www.youtube.com/embed/${getYouTubeID(modalVideo.videoUrl)}?autoplay=1`}
                    width="100%"
                    height="100%"
                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                    allowFullScreen
